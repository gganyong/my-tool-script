...
#head_url=http://shlinux22.ap.freescale.net/internal-only/Linux_IMX_Rocko_Legacy/209/
head_url=http://shlinux22.ap.freescale.net/internal-only/Linux_IMX_Full/latest/
...
dl_dtb_url=$head_url/common_bsp/zImage-imx6dl-sabresd.dtb
...
wget $dl_dtb_url -O $download_dir/imx6dl-sabresd.dtb
wget $yocto_url
name=`grep -o "test-internal-qt5-imx6qpdlsolox-[0-9]*.rootfs.tar.bz2" index.html | awk 'NR==1'`
#name=`grep -o "imx-image-restricted-imx6qpdlsolox-[0-9]*.rootfs.tar.bz2" index.html | awk 'NR==1'`
rm index.html
echo name=:
echo $name
